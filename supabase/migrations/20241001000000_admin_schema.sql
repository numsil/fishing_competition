-- 관리자 대시보드용 스키마
-- users/posts 컬럼 추가, post_videos 스토리지, reports/tickets/score_logs/badges 테이블
...
CREATE POLICY "Anyone can read post videos" ON storage.objects
  FOR SELECT USING (bucket_id = 'post_videos');
...
CREATE POLICY "Authenticated users can upload post videos" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id = 'post_videos' AND auth.role() = 'authenticated');
...
-- RLS 정책 (관리자는 service_role key로 bypass)
