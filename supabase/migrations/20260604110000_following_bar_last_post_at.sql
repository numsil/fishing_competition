-- 즐겨찾기 바: 최근 게시 순 정렬
-- last_post_at(피드 게시물 기준) 컬럼 추가, ORDER BY last_post_at DESC NULLS LAST
-- 반환 타입이 바뀌므로 DROP 후 재생성

DROP FUNCTION IF EXISTS public.get_following_with_max_score(uuid, int);
...
AS $$
  ...
  SELECT
    ...
    -- 가장 최근 피드 게시물 시각 (리그 전용·개인기록 제외)
    (
      SELECT MAX(created_at)
      FROM posts
      WHERE user_id = b.id
        AND league_id IS NULL
        AND is_personal_record = FALSE
        AND (is_deleted IS NULL OR is_deleted = FALSE)
    ) AS last_post_at
  FROM base b
  ORDER BY last_post_at DESC NULLS LAST, b.followed_at DESC;
$$;
